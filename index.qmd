...
# Load the schedule and circuits for the season
race_schedule <- load_schedule(season)
circuits_data <- load_circuits()

# Define circuit types (add more classifications as needed)
circuit_types <- circuits_data %>%
  mutate(
    type = case_when(
      circuit_name %in% c("Circuit de Monaco", "Baku City Circuit") ~ "Technical Circuit",
      circuit_name %in% c("Silverstone Circuit", "Red Bull Ring") ~ "High-Speed Circuit",
      TRUE ~ "Other"
    )
  )

# Ensure `laps_data$season` matches the type of `race_schedule$season`
laps_data <- laps_data %>%
  mutate(season = as.character(season))  # Convert laps_data$season to character
...
ggplot(heatmap_data, aes(x = team, y = avg_lap_time, group = team, color = team)) +
  ...
  facet_wrap(~ circuit_name, scales = "fixed", labeller = label_wrap_gen()) +  # Ensure consistent scaling
  labs(
